...
@Service
@Repository
public interface EmbarqueRepository extends JpaRepository<EmbarqueDesembarque, Integer> {
    ...
    List<EmbarqueDesembarque> findEmbarqueIdFuncionario (Integer id);
    ...
    List<EmbarqueDesembarque> findEmbarqueByAll (Integer id, Date dataInicial, Date dataFinal);
    ...
    EmbarqueDesembarque findUltimoEmbarqueAberto (Integer id);
    ...
    @Query (value = "select * from (select max(id) as id from embarque where funcionario_id_funcionario=? and data_desembarque is not null) X " +
            "inner join embarque emb on emb.id=X.id", nativeQuery = true)
    EmbarqueDesembarque findUltimoEmbarqueFechado (Integer id);
